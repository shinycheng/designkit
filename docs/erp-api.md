...
| `callback_url` | 任务完成后平台回调此地址（http/https）。平台默认允许内网地址；若平台在「系统设置」里关闭了内网访问，内网回调地址会被拒绝并记为 `rejected_unsafe_url` |
...
> `succeeded`（避免丢弃已计费的结果）。请以 `images.length` 为准，需要补齐时再发一次任务。
