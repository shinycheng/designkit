...
> ⚠️ **这份指南的定位：局域网测试环境。**（2026-08-11 更新）
> 项目的**正式部署目标已改为公网服务器**（有域名、HTTPS、反向代理、可自助注册），
> 步骤见 [README 第五章](../README.md#五部署到服务器)。群晖这一套继续保留、继续维护，
> 用来把功能跑顺、把备份练熟，以及给同一局域网里的同事和 ERP 用。
>
...
> **真要把这台 NAS 暴露到公网**（不推荐，但如果你决定这么做）：
> README 第五章「公网服务器上必须做的四件事」那一节的每一条都要做，
> 一条都不能省——尤其是 `DESIGNKIT_TRUSTED_PROXY_HOPS`
> （套了群晖自带的反向代理就填 `1`，不填的话全站所有人共用一个限速桶）。
