...
> 系统还有另一条路——发**邀请码**让对方自己注册（README 第八章）。
> **但在群晖这个内网环境里，建议就用「管理员建号」这一条路。**
> 自助注册是为公网服务器准备的：它要求先关掉「允许对外接口访问内网地址」，
> 而群晖这套的价值恰恰在于「和生图网关、和 ERP 在同一个内网」。
...
- **不要**把 8787 端口转发到公网（路由器端口映射）。当前是 HTTP 明文，
  ...
  或加一层反向代理配 HTTPS。
- **不要在群晖这套上打开「自助注册」。** 它默认就是关的，而且系统会强制要求
  先关掉「允许对外接口访问内网地址」才让你开——那一项正是群晖这套的价值所在
  （和生图网关、和 ERP 在同一个内网）。要开放注册请走公网服务器那条路，
  见 [README 第八章](../README.md#八让别人自己注册邀请码)。
