...
func TestAdminComplianceGuardBlocksAdminRouteWhenMissing(t *testing.T) {
	...
	// designkit（2026-08-13）：本实例已去掉合规确认关卡。
	// 这道中间件本身没删（万一以后想开回来），但 GetAdminComplianceStatus
	// 现在恒返回 Required=false，所以它一律放行——原来断言的是 423 + 错误码。
	//
	// 用例保留而不是删掉：它守着「关卡确实是关着的」这件事。
	// 哪天有人把 Required 改回 true，这里会红，提醒他 routes/admin.go 里
	// 那行 admin.Use(AdminComplianceGuard(...)) 还是注释掉的状态。
