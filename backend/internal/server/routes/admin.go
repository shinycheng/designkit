...
func RegisterAdminRoutes(
	...
	// designkit（2026-08-13）：去掉「部署与运营合规确认」这道关卡。
	//
	// 上游用它做责任隔离：管理员进控制台前必须逐字输入确认短语，
	// 系统记下版本、时间、账号、IP、UA 作为留痕。
	// monica 是本实例唯一的部署与运营主体，明确表示不需要这个流程。
	//
	// ⚠ 这里去掉的只是**弹窗和拦截**，不是义务本身：
	// 备案、上游服务条款、内容安全、数据保护这些是法律和合同规定的，
	// 跟这道关卡在不在没有关系。上游 README 的免责声明和作者署名一律保留。
	//
	// 刻意**没有**去数据库里写一条「已确认」记录——那条记录会带上
	// 账号 ID、IP 和时间戳，等于替她签一份她没签过的东西。
	// 要去掉就诚实地把关卡摘掉。
	//
	// admin.Use(middleware.AdminComplianceGuard(settingService))
	{
		// 合规确认的接口保留（GET 状态 / POST 确认）：删掉的话前端 store
		// 拉状态会 404，反而要连带改前端。留着它们不会主动弹任何东西。
