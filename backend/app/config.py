...
RUNTIME_DEFAULTS = {
    ...
    #
    # ⚠ 这一项和下面的「自助注册」是**互斥**的，接口层必须双向卡死：
    #    开着自助注册 → 这一项只能是 False，而且不许再改回 True；
    #    这一项是 True → 不许打开自助注册。
    # 理由：自助注册意味着陌生人能自己拿到 ERP Key，而 v1 接口的 image_urls /
    # callback_url 是由调用方指定地址、**由服务端去访问**的。放开内网等于把
    # 「让 designkit 替我访问同一个内网里的任意 http 地址」这个能力发给了所有人——
    # 包括那台 Sub2API 的管理端口，上面挂着能建号、改余额、读任意用户明文 API Key
    # 的接口。默认值故意保持 True 不动：内部 ERP 常在内网，改默认会静默打断现有部署。
    ...
    # ══════════════════════════════════════════════════════════════
    #  限速（services/ratelimit.py）
    # ══════════════════════════════════════════════════════════════
    # 信任几层反向代理。**默认 0 = 没有反代**，这时按 TCP 连接的对端地址算。
    # 群晖上套了一层反向代理（或 Nginx / Caddy）就填 1，前面还有 CDN 就填 2。
    # 填错的后果不对称，所以宁可少不可多，详细解释见 deps.py 的 client_ip：
    #   填少了 → 所有人共用一个限速桶（严一点，但没有安全问题）；
    #   填多了 → 会去信客户端自己伪造的那一段地址，限速当场作废。
    "trusted_proxy_hops": int(os.environ.get("DESIGNKIT_TRUSTED_PROXY_HOPS", "0")),
    # 登录限速：15 分钟内失败 10 次，之后锁 15 分钟。
    # 这三个数与改造前写死在 routers/auth.py 里的完全一致，升级当天行为不变。
    # 键名的格式是 ratelimit_<scope>_xxx，services/ratelimit.py 按这个约定去找；
    # 以后加短信验证码、开放注册，照着补 ratelimit_sms_code_* / ratelimit_register_*
    # 三行就行，那边一行代码都不用改。
    "ratelimit_login_max_attempts": int(os.environ.get("DESIGNKIT_RATELIMIT_LOGIN_MAX", "10")),
    "ratelimit_login_window_minutes": int(
        os.environ.get("DESIGNKIT_RATELIMIT_LOGIN_WINDOW_MINUTES", "15")
    ),
    "ratelimit_login_block_minutes": int(
        os.environ.get("DESIGNKIT_RATELIMIT_LOGIN_BLOCK_MINUTES", "15")
    ),
    # 生图网关的计费方式：
    ...
    # ══════════════════════════════════════════════════════════════
    #
    # 这一组和上面的「网关自动开通」一样，**故意一个都不读环境变量**，
    # 全部由管理员在「系统设置 → 自助注册」里填。这里的理由比那边更硬：
    # 总开关是一道安全闸门，打开它之前接口层要先确认几件事（见下面
    # self_register_enabled 的说明）。留一个 .env 后备，等于留了一条
    # **绕过这些检查**把注册打开的路——而且是那种「谁也想不起来自己什么时候
    # 设过这个变量」的路。想开就到界面上开，检查才会跑到。

    # 自助注册总开关。**默认关闭 —— 这个功能上线当天不改变任何现有行为。**
    #
    # 打开之前接口层必须**强制**确认（这是前提，不是建议）：
    #   ① allow_internal_targets 必须是 False（理由见它自己那一条），
    #      而且开着注册期间不许再改回 True —— 闸门是双向的；
    #   ② 关掉之后再打开也要重新确认一遍，别把它做成「只在第一次打开时检查」。
    # 关掉这个开关不会影响任何已经注册成功的人，只是不再接受新注册；
    # 已经发出去的邀请码也不会失效，重新打开后照旧能用（要彻底停就去作废码）。
    "self_register_enabled": False,

    # 同一个 IP 每小时最多**尝试**注册几次（成功失败都算）。
    # 挡的是「拿一批码或一张多次码来批量刷号」的脚本。
    # 20 次是权衡：一个真人从看到码到注册成功，撑死试三五次（码打错、
    # 用户名被占、密码太短）；给到 20 还留了同一间办公室多人共用一个出口 IP 的余地。
    #
    # ⚠ 群晖上套了反代（Nginx / Caddy / 群晖自带反向代理）时，后端看到的
    # request.client.host 是**反代自己的地址**，全站所有人共用一个桶——
    # 这时这个阈值实际上退化成了「全站每小时最多试 20 次」，会把正常用户一起挡住。
    # 真上公网之前，要么在反代层做限速，要么让后端认 X-Forwarded-For
    # （认之前必须先确认反代会覆写这个头，否则它是攻击者随便填的，等于没限速）。
    "self_register_ip_per_hour": 20,

    # 全站每天最多**成功**注册多少个账号。
    # 这是最后一道兜底：万一某张码的次数填错了（比如手滑填成 1000），
    # 或者限速被绕过，损失也就止于这个数，第二天零点自动恢复。
    # 每多一个账号就多一个要开通网关、要花钱的人头，所以这个数字宁可小。
    "self_register_daily_limit": 50,

    # ── 管理员在界面上新建邀请码时的默认值（只是预填，创建时可改）──
    # 默认「一张码只能用一次」：谁用了这张码一目了然，出事时能精确定位到人。
    # 想一次拉一批人进来，就多建几张码，别把一张码的次数调大——
    # 次数大的码一旦被转发到群里，你只知道有 20 个号是它开的，不知道该停哪个。
    "invite_code_default_max_uses": 1,
    # 默认 7 天有效。**不要默认永久**：永久有效的码等于一张躺在库里的长期通行证，
    # 发出去之后就再也收不回来了（除非管理员记得回来作废，而没人会记得）。
    "invite_code_default_valid_days": 7,

    # ══════════════════════════════════════════════════════════════
    #  自助注册（手机号 + 短信验证码）
    # ══════════════════════════════════════════════════════════════
    #
    # 和上面两组一样，**这一组一个都不读环境变量**。理由在这里最硬：
    # 这里既有安全闸门（总开关、限速阈值），又有密钥（AccessKey）。
    # 留 .env 后备等于留了一条「绕过界面检查把注册打开」的路，
    # 而且是那种谁也想不起来自己什么时候设过的路。想开就到界面上开。
    #
    # ⚠ 短信和邀请码有一个本质区别：**发一条短信就是一笔真金白银的支出**，
    # 而且被刷狠了阿里云会以「疑似恶意」为由把签名封掉——那时候不是花钱的问题，
    # 是整个手机号注册通道直接停摆，重新申请签名要按工作日算。
    # 所以下面四个限速阈值不是可有可无的调优项，是这条通道能不能上线的前提。

    # ── 短信通道 ──
    # 用哪种方式把验证码送出去：
    #   debug   —— **调试模式，默认值**。根本不发短信，验证码写进服务端日志、
    #              同时直接显示在网页上，用来把整条注册流程走通。
    #   aliyun  —— 阿里云短信服务，真的发、真的花钱。
    #
    # ⚠ 界面上必须做到两件事，缺一不可（这是本期的硬要求）：
    #   ① 处在 debug 时，发码页面上要**显著**标明「这是调试模式，验证码没有
    #      真的发出去」。不标的话，哪天管理员忘了切回 aliyun，会一直以为短信
    #      发出去了，而用户那边永远收不到，双方都查不出问题在哪。
    #   ② 切到 aliyun 之前，接口层要先确认下面四项（AccessKey Id / Secret /
    #      签名 / 模板 code）都填了——没填就切过去，表现是每个人点「获取验证码」
    #      都失败，而失败原因是阿里云返回的一串英文错误码。
    "sms_provider": "debug",
    # 阿里云 AccessKey Id。它更像「用户名」而不是密码，所以**故意不打码**：
    # 管理员要能一眼看出自己填的是哪一把（同一个阿里云账号下常有好几对 Key）。
    # 真正的密码是下面那一项。
    "sms_aliyun_access_key_id": "",
    # 阿里云 AccessKey Secret。**必须在 SECRET_SETTING_KEYS 里全星号打码**
    # （见本文件末尾），并且**必须用 services/secrets_box.py 加密之后再落库**。
    # 这里永远是空串：不预填、不读 env、不写进任何模板文件或测试数据。
    #
    # 加密落库这一条要特别说明，因为它和现有的 sub2api_admin_key 不一样：
    # 那一把今天是明文存在 app_settings 里的（只在接口返回时打码）。
    # 这一把不行——它是**阿里云账号级别**的凭据，拿到手不只是能发短信，
    # 取决于这个 RAM 用户的授权范围，还可能碰到别的云产品。
    # 约定：写设置时存 secrets_box.encrypt(明文)，发短信时 secrets_box.decrypt。
    "sms_aliyun_access_key_secret": "",
    # 短信签名，形如「XX电商」。必须和阿里云控制台里审核通过的那个**逐字一致**，
    # 差一个字就是发送失败（错误码 isv.SMS_SIGNATURE_ILLEGAL）。
    "sms_aliyun_sign_name": "",
    # 短信模板 CODE，形如 SMS_123456789。模板正文里必须有 ${code} 这个变量，
    # 否则验证码发出去是一条没有码的短信。
    "sms_aliyun_template_code": "",
    # 阿里云短信的接口域名。**正常情况下不要改**，留这一项是为了两件事：
    # ① 自动化测试可以把它指到本机的假上游，从而验证签名算法而不真的发短信、
    #    不真的花钱（本项目禁止在测试里调用真实短信接口）；
    # ② 万一以后要走 VPC 内网域名或换区域，不用改代码。
    "sms_aliyun_endpoint": "dysmsapi.aliyuncs.com",

    # ── 手机号注册总开关 ──
    # **默认关闭 —— 这个功能上线当天不改变任何现有行为。**
    #
    # 它和邀请码注册的 self_register_enabled 是**并列**的两条路，共用同一套
    # 「开放注册的硬前置闸门」（见 routers/register.py 的文件头第二条）：
    #   ① allow_internal_targets 必须是 False；
    #   ② 对外访问地址（public_base_url）必须填对；
    #   ③ 取图鉴权（files_signed_only）必须开着。
    # 每一次注册请求都要重新确认一遍这个组合，不是只在打开开关那一刻查一次。
    #
    # 打开它之前还要额外确认一条（这是手机号这条路独有的）：
    # 短信通道要么处在 debug（不花钱），要么 aliyun 的四项都填齐了。
    # 否则打开的是一扇「所有人都会看到红字报错」的门。
    "phone_register_enabled": False,

    # 手机号注册的时候，要不要**同时**填一张邀请码。**默认要求填（先紧后松）。**
    #
    # 开着（默认）：手机号只负责证明「这个号是你的」，能不能进来还是由邀请码说了算。
    #   适合「我知道要拉哪几个人进来」的场景——把码发给他们，别人拿不到码就进不来。
    # 关掉：**任何人只要有一个能收短信的手机号就能注册进来**，没有任何名单限制。
    #   只有在真的想对外开放（比如做一次公开试用）时才关，而且关之前要先确认
    #   「全站每天最多新增账号数」和三层短信限速都设成了你能承受的数字——
    #   每一个新账号都要去网关开通、都要花钱，每一条验证码短信也要花钱。
    #
    # 为什么默认是「要求填」：这两个方向的代价完全不对称。默认严一点，最坏是
    # 管理员多点一下开关；默认松一点，最坏是某天有人把手机号注册打开，
    # 于是全网都能注册进来——而这件事发生的时候不会有任何报错，
    # 界面上、日志里都看不出异常，等发现时账号已经进来了一批。
    #
    # ⚠ 这一项**必须留在 RUNTIME_DEFAULTS 里**：services/settings_service.set_many
    # 会把 RUNTIME_DEFAULTS 里没有的键**静默丢掉**（不报错），删掉它的表现是
    # 设置页上这个开关怎么点都存不进去，而页面还显示「已保存」。
    "phone_register_require_invite": True,

    # ── 验证码本身 ──
    # 验证码有效期（秒）。300 秒 = 5 分钟：短信到达一般在 10 秒内，
    # 5 分钟足够一个人从收到短信到切回浏览器填进去；再长就等于给
    # 「慢慢猜 6 位数」多留时间。到期时刻在发码那一刻就钉死在数据库里
    # （见 models.PhoneVerificationCode.expires_at），改这一项只影响以后发的码。
    "sms_code_ttl_seconds": 300,
    # 同一条验证码最多能试错几次，超了就作废、要求重发。
    # 5 次是权衡：真人填错一两次很常见（看错数字、串了行），
    # 但 6 位数只有 100 万种可能，不限次数地猜是猜得完的。
    "sms_code_max_attempts": 5,

    # ── 三层限速（每一层都必须走 services/ratelimit.py，不许自己造）──
    #
    # 为什么是三层、少一层会怎样：
    #   只限手机号 → 攻击者拿一万个手机号轮着发，每个号都没超限，
    #                但一晚上就是一万条短信的账单（而且全是骚扰真实机主）。
    #   只限 IP    → 换一批代理 IP 就绕过去了，成本几乎为零。
    #   只限全站   → 一个人就能把当天的名额占光，正常用户全被挡在门外。
    # 三层叠在一起，任何一条路径被打满都会先撞上其中一堵墙。

    # 第一层之一：同一个手机号两次发送之间至少隔多少秒。
    # 60 秒是行业惯例（用户对「60 秒后重发」这个倒计时有预期），
    # 也是挡住「按住重发按钮连点」这种最常见的浪费。
    "sms_code_resend_cooldown_seconds": 60,
    # 第一层之二：同一个手机号一天最多收几条。
    # 10 条已经很宽松了（真人一天注册一次撑死重发两三回）；
    # 它挡的是「拿一个号当靶子反复发，把机主骚扰到投诉」——
    # 被投诉的后果是阿里云直接封签名，比花掉的钱严重得多。
    "sms_code_phone_daily_limit": 10,
    # 第二层：同一个来源 IP 每小时最多发几条（不同手机号也一起算）。
    # 20 条留了「同一间办公室多人共用一个出口 IP」的余地。
    # ⚠ 套了反代时全站会共用一个桶（见 trusted_proxy_hops），公网部署前务必配对。
    "sms_code_ip_hourly_limit": 20,
    # 第三层：全站一天最多发几条，最后一道兜底。
    # 200 条对一个内部电商团队来说是绰绰有余的量级；万一前两层被绕过、
    # 或者哪个阈值被填错，损失也就止于这个数（阿里云国内短信约 4 分/条，
    # 200 条 ≈ 8 块钱），第二天自动恢复。
    # 名额用完时给用户的提示里要写清楚「多久之后恢复」，并告诉他可以联系管理员
    # 直接建号——照 self_register_daily_limit 那一条的写法。
    "sms_code_global_daily_limit": 200,
}
...
#
# 这里登记的项会被全星号打码（连末 4 位都不给），因为它们泄露一次的后果
# 都不是「多花点钱」级别的：sub2api_admin_key 是整台网关的最高权限，
# sms_aliyun_access_key_secret 是阿里云账号级别的凭据。
# 界面上只需要表达「填了没有」，不需要泄露其中任何一部分。
#
# ⚠ sms_aliyun_access_key_id **故意不在这里**：它相当于用户名，
# 管理员要能看见自己填的是哪一把（同一个阿里云账号下常有好几对 Key）。
SECRET_SETTING_KEYS = ("sub2api_admin_key", "sms_aliyun_access_key_secret")
