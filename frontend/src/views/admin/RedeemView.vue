<template>
  <AppLayout>
      ...
      <template #pagination>
        ...
        >
          ...
          <div class="flex flex-wrap items-center gap-2">
            ...
            <!--
              批量删除。放在批量修改右边、用危险色：删掉的兑换码**找不回来**，
              而它跟旁边那个「批量修改」只差一个词，不给视觉区分很容易点错。
            -->
            <button
              ...
              @click="openBatchDeleteDialog"
            >
              {{ batchDeleting ? t('common.loading') : t('admin.redeem.batchDelete') }}
            </button>
          </div>
        </div>
    ...
    <!--
      批量删除的确认。
      ⚠ 一定要把**条数**写进去（selectedCount）：只说「确定删除吗」的话，
      管理员不知道自己选中的是 3 条还是 300 条 —— 而勾选状态是跨页保留的
      （selectedCodeIds 是个 Set），很容易比他眼前看到的多。
    -->
    <ConfirmDialog
 ...
 * 批量删除选中的兑换码。
 *
 * 后端 POST /admin/redeem-codes/batch-delete 早就有了（跟「删除所有未使用」
 * 用的是同一个接口），缺的一直只是界面上的入口。
 *
 * ⚠ 删完必须 clearSelectedCodes()：选中集合是跨页保留的，不清的话
 * 那些已经删掉的 id 还留在里面，管理员接着点「批量修改」会对着一批
 * 不存在的 id 发请求。
 *
 * ⚠ 后端返回的 deleted 可能**小于**选中数（有的已经被别人删了 / 已使用不让删）。
 * 提示语报的是**真实删掉的条数**，不是选中数 —— 报选中数会让管理员
 * 以为都删干净了。
 */
