export default {
    ...
    redeem: {
      ...
      batchUpdate: '批量修改',
      batchDelete: '批量删除',
      // ⚠ 必须把条数写进去：勾选是跨页保留的，管理员眼前看到的可能只是其中一部分。
      // 而且要说明「删掉找不回来」——兑换码删了就没了，没有回收站。
      batchDeleteConfirm: '确定要删除选中的 {count} 个兑换码吗？删掉之后找不回来。',
      // 跟「删除全部未使用」那句分开：批量删除的这些不一定都是未使用的。
      // 报的是**真实删掉的条数**，可能比选中的少（有的已被删、有的已使用不让删）。
      batchDeleted: '已删除 {count} 个兑换码',
