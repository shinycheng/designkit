<template>
  ...
  >
    ...
    <div class="mt-auto border-t border-gray-100 p-3 dark:border-dark-800">
      <!-- designkit: 主题切换按钮已移除——全站只有 DesignKit Dark 一套主题
           （monica 2026-08-15 拍板 D2「单主题不留开关」）。深色由
           features/designkit/theme/force-dark.ts 钉死；这里留个按钮只会让
           运营点了没反应（class 被 observer 立刻加回来），比没有更糟。
           原先靠 light-only.css 的 :first-child 选择器隐藏（自己都标注是
           「最脆的一条」），直接删按钮才是正解。 -->
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  // designkit: 自建的五个页面排在最前（运营的日常入口），管理员在「我的账户」区也能看到
  items.push(
    ...
    { path: DESIGNKIT_CONTENT_CHECK_PATH, label: t('designkit.nav.contentCheck'), icon: DesignkitContentCheckIcon },
...
// finalizeNav 合并三重过滤：featureFlag 过滤 + simple 模式过滤 + designkit 角色过滤。
function finalizeNav(items: NavItem[]): NavItem[] {
  const visible = applyFeatureFlags(items)
  const afterSimpleMode = authStore.isSimpleMode ? visible.filter(item => !item.hideInSimpleMode) : visible
  // designkit: 普通用户（电商运营）只留我们自建的菜单 + 个人资料；管理员看得到全部。
  // 本地过滤，不走 featureFlags（新增一个 flag 要改 11 个文件，10 个是上游文件）。
  if (isAdmin.value) {
    return afterSimpleMode
  }
  return afterSimpleMode.filter(item => !isUpstreamNavHiddenFromUser(item.path))
}
...
// designkit: 主题初始化已由 features/designkit/theme/force-dark.ts 接管（全站深色单主题）。
