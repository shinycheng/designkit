...
export default defineConfig(({ mode }) => {
  ...
  build: {
    ...
    server: {
      host: '0.0.0.0',
      port: devPort,
      // 允许用哪些域名访问**开发服务器**。默认空 = 只能用 IP 或 localhost 访问，
      // 换成域名会被 Vite 拒掉并提示 "This host is not allowed"。
      //
      // ⚠ 这道拦截是**保护**，不是障碍：开发服务器的 /@fs/ 能读到项目目录下的
      // 任意文件，绝不该出现在公网上。域名要对外，请指向后端端口（默认 8080，
      // 群晖上映射成 18080）—— 那是内嵌了打包后前端的应用本体，
      // 没有这层限制，也不存在这个风险。
      //
      // 只有「想在局域网里用自定义域名调试热更新」时才填，多个用逗号分隔：
      //   VITE_DEV_ALLOWED_HOSTS=dk.lan,designkit.local pnpm dev
