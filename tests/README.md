...
每次改动代码后，跑一遍**全部 19 组**测试确认没把已有功能改坏，共 612 项：
...
三组端到端要起服务，另外十六组是纯单元测试（共 503 项），不联网、不花钱、不用起服务。
...
末尾出现 `Ran 503 tests ... OK (skipped=10)` 就是全过了
（那 10 项 skipped 是 PostgreSQL 迁移，见上面的 ⚠️）。

> **跑完大约要三四分钟，中间会安静好一阵子，不是卡住了。**（实测 503 项用了
> 210 秒。）慢的是三组「自动开通」，它们占了其中大半：每一项都真的起一台假网关、
