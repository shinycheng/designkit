...
| 阿里云短信签名（官方测试向量） | `test_sms_signing.py` | 8 | 不要 |

三组端到端要**你自己**先起一个服务，另外十七组一条命令就能跑完（共 531 项），
不联网、不花钱。

> ⏳ `test_multiprocess.py` 是这十七组里唯一一个**要等一分多钟**的。
> 它会自己把 uvicorn 拉起来（4 个工作进程）再自己收拾干净，**不用你动手**。
> 它慢是应该的，为什么值得等见下面「多进程单测」一节——
> **看着碍事就把它删掉，等于把「每张图多花一倍的钱」这条线上的守卫撤了。**

> **新增测试文件要回来把这张表补上**（含项数，并把上面那两个总数一起改对）。
> 这张表不是装饰：它是「跑完之后怎么知道自己跑全了」的唯一依据——
> 末尾那行 `Ran N tests` 对不上表里的数，就说明有一组没被 discover 到
> （文件名没以 `test_` 开头是最常见的原因），而那种情况**不会报错**，
> 只会安静地少跑一批。数怎么数见下面「十七组单元测试」一节。
...
末尾出现 `Ran 531 tests ... OK (skipped=10)` 就是全过了
（那 10 项 skipped 是 PostgreSQL 迁移，见上面的 ⚠️）。

> **跑完大约要五分钟，中间会安静好一阵子，不是卡住了。**（实测 531 项用了
> 281 秒。）慢的地方有两处：一是三组「自动开通」，它们占了其中大半：每一项都真的
> 起一台假网关、真的走一遍 HTTP，还要等几次退避和限流；二是「多进程」那一组
> （74 秒，其中有一项光是等租约过期就要 30 多秒，见「多进程单测」一节）。
> 屏幕上会滚过很多行红字，例如
...
> 数据库和历史生成记录。
>
> 忘了写也不会出事：不设这个变量时，整次运行会自己开一个临时目录（十六个文件共用
...
```bash
rm -rf /tmp/dk-unittest && DESIGNKIT_DATA_DIR=/tmp/dk-unittest DESIGNKIT_PROVIDER=mock \
  .venv/bin/python -m unittest discover -s tests -p 'test_invite_register.py' -v
```

- `test_invite_register.py` — 61 项，守「让陌生人不经管理员就能自己开户」这条链路。
  这是全系统**不需要登录的写接口**之一（另一条是手机号注册，见下面那一节），
  所以它的每一条防线都比别处更值钱。

六条线，每一条都对应一种**不会报错、手工点一遍全都正常**的事故：

1. **开放注册的硬前置闸门（最贵的一条）**——「允许对外接口访问内网地址」
   默认是**开着**的（为了让现有 ERP 能取内网里的图）。开着它，对外接口能让
   **服务端**去访问内网任意地址，包括同一内网里生图网关的管理端口——
   那上面挂着能建号、能改余额、能读任意用户明文 Key 的接口。
   而自助注册意味着陌生人能自己拿到一把 ERP Key，两者叠在一起就是
   「把内网的门连同钥匙一起发出去」。所以闸门必须**双向**：
   内网开着就打不开注册，注册开着就不许把内网改回来。
   坏掉的样子：设置页保存成功，界面一切正常，没有任何报错。
2. **邀请码并发扣减**——两个人同时用同一张只剩一次名额的码，只能成一个。
   ⚠ 这几项**必须真的开线程**（测试里用 `threading.Barrier` 让请求真的重叠）。
   改成「顺序调两次接口」会永远通过，也就永远发现不了「先查够不够、再写回去」
   这个错误写法——而那正是这几项存在的唯一理由。
3. **限速跨进程共用一个桶**——公网部署要开多 worker。限速如果记在进程内存里，
   设置里写「10 次」实际会变成「10 × 进程数」，而界面上完全看不出来。
   测试里**真的另起一个 python 进程**去撞，撞满之后回到本进程看还认不认账。
4. **客户端 IP 从 X-Forwarded-For 右边数**——最左边那一段是客户端自己写的。
   照最左边算限速，攻击者每次换一个假 IP 就等于没有限速。
5. **注册与开通解耦**——生图网关整个连不上时，注册照常成功、人照常登录进来，
   只是暂时不能生图。坏掉的样子：网关一停机，新人一个都进不来。
6. **报错不许变成探测器**——邀请码「不存在」和「已用完」回同一句话，
   用户名被占用不说「已存在」。分开说，这个接口就成了一台可以拿去枚举的判定器。

> **这一组每个用例自己起一个临时库**，不和别的组共用。原因是它要反复改
> **全局设置**（注册总开关、内网访问、限速阈值），而设置表是全库一行的——
> 共用一个库的话，同一次 discover 里排在后面的测试会莫名其妙地跑在
> 「注册已开放」或者「限速阈值是 3」的环境里，而且症状飘忽不定。

> 这一组也**不碰任何真实网关**。「网关全挂」是这么造出来的：起一台
> `fake_sub2api.py` 假网关再立刻关掉，把空出来的端口填进设置里
> ——连接被拒是毫秒级的，比等超时快得多，也比随手写一个端口号可靠
> （随手写的端口有可能正好被别的程序占着，那测出来的是「连上了但接口不对」，
> 和「网关关机了」根本是两回事）。
...
```bash
rm -rf /tmp/dk-unittest && DESIGNKIT_DATA_DIR=/tmp/dk-unittest DESIGNKIT_PROVIDER=mock \
  .venv/bin/python -m unittest discover -s tests -p 'test_phone_register.py' -v
```

- `test_phone_register.py` — 39 项，守「手机号 + 短信验证码自助注册」这条链路
  （四条免登录接口：`GET /api/web/phone/status`、`POST /code`、`/register`、`/login`）。
  **不联网、不发短信、不花钱**：调试通道（`sms_provider=debug`）本来就不发短信。

这是全系统**另一条不需要登录的写接口**（上一节那条是邀请码注册），
而且比邀请码那条更贵——邀请码被刷只是多几个账号，
短信被刷是**每一次点击都从你的阿里云余额里扣钱**。
七条线，每一条都对应一种**不会报错、手工点一遍全都正常**的事故：

1. **关着的时候要真的关死**——总开关默认关闭，而且和邀请码注册**共用**同一道
   「开放注册的硬前置闸门」（内网访问必须关、对外地址必须填对、图片凭证必须开）。
   坏掉的样子：管理员把邀请码注册关掉、只开手机号注册，就绕过了整道闸门。
2. **超限时一条短信都不许发出去**——注意是「不许发」，不是「发了再拦」：
   拦在发送之后，钱已经花掉了。而且额度必须在**真正调用上游之前**扣掉，
   否则「让每次调用都超时」就能无限次花钱而配额一次都不减。
3. **不许泄露某个手机号注册过没有**——发码接口对已注册和未注册的号码，
   响应逐字一样。分开说，这个接口就成了一台可以拿去枚举「谁是这里的用户」的判定器。
4. **验证码只存哈希**——拿到数据库也反查不出码。
5. **试错次数有上限**，用完当场作废（6 位数只有 100 万种，不限次数是猜得完的）。
6. **注册与开通解耦**——生图网关整个连不上时，注册照常成功、人照常登录进来，
   只是暂时不能生图。注册接口本身一步外部请求都不发。
7. **邀请码原子扣减**——同时要求填邀请码时（默认就是这样），
   一张一次性码不能开出两个号。

### 里面的「隐私回归」四项（`PhonePrivacyTests`，**不许删**）

这四项守的是一个已经发生过、并且已经修掉的问题：
**手机号原本会以明文进限速表和服务端日志。**

为什么这件事必须长期有测试盯着，而不是修完就算：

- `rate_limit_state` 是**长期留存**的表，而且会为「每一个曾经请求过验证码的号码」
  留一行——包括打错的、试探的、发完再没回来的。它会慢慢攒出一份
  「所有来过的手机号」名单，而这些人绝大多数从没注册成功，
  系统本来就没有任何理由保存他们的号码。
- 日志文件是会被打包发给别人排错的，所以「打进日志」等于「发出去了」。
- **这两件事都不会报错**：功能完全正常，限速也照常生效，界面上、日志里
  都看不出任何异常。下一个人「顺手把桶名改回手机号方便排查」就能把它打开。

现在的做法与这四项断言的东西：
①限速桶名是**带密钥的哈希**，里面找不到号码的任何一段；
②同号算出同一个桶（否则限速形同虚设）、不同号必须分开；
③**必须带密钥**——中国大陆手机号不到百亿种，裸 `sha256` 几分钟就能反查回明文；
④第二道保险：`ratelimit.mask_key` 把日志里的长数字串打码成 `138****8000`，
　同时**不许误伤 IP**（`192.168.31.235` 要原样保留，排错全靠它）。

## 短信设置与「试发一条」单测

```bash
rm -rf /tmp/dk-unittest && DESIGNKIT_DATA_DIR=/tmp/dk-unittest DESIGNKIT_PROVIDER=mock \
  .venv/bin/python -m unittest discover -s tests -p 'test_sms_settings.py' -v
```

- `test_sms_settings.py` — 45 项，守设置页「短信服务（手机号注册）」那一节：
  怎么保存、怎么打码、什么时候不许切到真实通道，以及那个「试发一条」按钮。

**绝不调用真实的阿里云接口**——调一次就是花钱。「试发」那几组一律把设置里的
`sms_aliyun_endpoint` 指到本机回环上的一个假上游（`services/sms.py` 留这个
设置项就是为了这件事），由它按用例需要返回 `Code: OK` 或某个错误码。
测试数据里的 AccessKey 全是编出来的字符串，**与任何真实凭据无关**。

五条线：

1. **超限时一条都不许发出去**——四层限速（同号冷却、同号每天、同 IP 每小时、
   全站每天）的顺序必须是「四层全部只读检查通过 → 再一起扣额度 → 最后才发」。
   坏掉的样子：额度对不上账（第四层拦下来时前三层已经白扣了），
   或者更贵的一种——两次点击同时通过检查，发出去两条。
2. **AccessKey Secret 必须加密落库、打码回吐、原样回传视为「没改」**。
   坏掉的样子：管理员保存一次别的设置，Secret 就被一串星号覆盖掉了，
   而页面还提示保存成功，直到下一个人点「获取验证码」才发现发不出去。
3. **最常犯的几种手滑要在保存那一刻就被拦住**——把输入框里那句灰色提示文字
   粘进来（这个坑在「Sub2API 管理员 Key」那一栏真实发生过一次）、复制时多带了
   一行、把模板正文当成模板 CODE 填进来。坏掉的样子：保存成功、界面正常，
   然后每个人点「获取验证码」都失败，失败原因是一串英文错误码。
4. **闸门是双向的，而且两条注册路共用**（同上一组的第 1 条）。
5. **调试模式必须在界面上看得出来**——不标的话，哪天忘了切回真实模式，
   管理员会一直以为短信发出去了，而用户那边永远收不到。
   **双方都查不出问题出在哪，这种故障没有人能自查。**

顺带还盯着几个错误码的提示语是不是人话：余额不足要说「去充值」、
签名或模板没审核通过要指向控制台的审核状态、被流控要说「等一会儿再试」。

## 阿里云短信签名单测

```bash
rm -rf /tmp/dk-unittest && DESIGNKIT_DATA_DIR=/tmp/dk-unittest DESIGNKIT_PROVIDER=mock \
  .venv/bin/python -m unittest discover -s tests -p 'test_sms_signing.py' -v
```

- `test_sms_signing.py` — 8 项，只做一件事：把签名算法对着**阿里云官方文档
  给出的那组示例数据**逐字核一遍。**不发短信、不联网、不花钱。**

**为什么这 8 项值得单独一个文件长期跑**：签名算错的表现是**一直 403，
而错误信息看不出是哪一步错了**——百分号编码差一个字符、拼接顺序反了、
密钥末尾少一个 `&`、AccessKey 完全填错，阿里云报的是同一句话。
真到线上查这种问题，要么花钱一条条发短信试，要么对着一堆百分号肉眼比对，
而且**改错一个字符和什么都没改，症状一模一样**。

官方那组示例给了确定的 AccessKeySecret 和期望签名（见阿里云帮助中心
「RPC 签名机制」）。只要这一组对得上，编码规则、拼接顺序、密钥后缀、
摘要算法就全对。剩下 5 项盯的是最容易写错的编码细节：
空格必须编成 `%20` 而不是 `+`、`*` 要编、`~` 不许编、`/` 要编、中文按 UTF-8 编。

> 向量用的是 ECS 的 `DescribeDedicatedHosts`，不是短信接口——**这不是笔误**。
> 签名机制是所有 RPC 风格接口共用的，换成短信只是参数不同。
> 这正是它能当测试向量的原因：不用真发一条短信（那要花钱），就能证明签名对。
...
拿一把他自己的 Key」这条链路。三组各盯一段，共 121 项：

- `test_provisioning.py` — 54 项，**状态机内部**：七个状态之间每一条转移、
  每一种失败的分类与退避排期、409 之后怎么反查认领、撞上抢注怎么换算。
- `test_provision_regression.py` — 40 项，**从接口进、从接口出的最终表现**：
...
## 多进程单测（公网多 worker 部署，⏳ 慢的那一组）

```bash
rm -rf /tmp/dk-unittest && DESIGNKIT_DATA_DIR=/tmp/dk-unittest DESIGNKIT_PROVIDER=mock \
  .venv/bin/python -m unittest discover -s tests -p 'test_multiprocess.py' -v
```

- `test_multiprocess.py` — 28 项，**实测 74 秒**。不联网、不花钱、不用你起服务
  （它自己把 uvicorn 拉起来，跑完自己收拾干净，也不会碰你的 `data/`）。

### 为什么非得单独有这一组

**这一组能抓到的问题，别的 500 多项一个都抓不到**——因为它们全都跑在
**一个进程**里，而这些问题只在「同一份代码被好几个进程各跑一遍」时才出现。

公网服务器要开多个 uvicorn 工作进程（`DESIGNKIT_WORKERS=4`）。uvicorn 的每个
工作进程是一个**独立的操作系统进程**，各自完整跑一遍启动流程。于是：

1. **同时生成几张图会被悄悄乘上进程数（最贵的一条）。**
   管理员在设置页填「同时生成 2 张」，开 4 个进程实际是 **8 张同时在跑**，
   而**界面上显示的还是 2**。每一张图都是真金白银，网关那边还可能有并发上限，
   一超就整批报错——运营看到的只是「今天怎么老是生成失败」，完全无从查起。
   这一组会真的起 4 个进程、真的丢一批任务进去，每 50 毫秒数一次
   「此刻有几张在跑」，再用「按每条任务的起止时间算最大重叠」印证一遍。
   **改造之前的代码在这里量到的是 6 张**（本机实测），改造之后是 2 张。
2. **派发的那个进程被杀之后，没人接手，从此不出图。**
   容器被 OOM 杀、NAS 断电、部署滚动重启，都是这个形态。
   现在的设计是「谁拿到数据库里那把租约谁派发」，租约 30 秒、待命进程 5 秒
   轮询一次，所以**最坏 35 秒**有人顶上。这一组真的 `kill -9` 掉正在派发的
   那个进程，量接手用了多久、接手之后并发上限还成不成立。
3. **全新安装时四个进程同时建库**（建表、写种子数据）。
4. **限速被进程数除了**——设置里写「3 次」，四个进程各记各的就成了 12 次。
5. **真实客户端 IP 取错**——套上反向代理后，uvicorn 自己会改写来源地址，
   应用又会再算一遍 `X-Forwarded-For`。两层叠加的结果**只有真的起 uvicorn
   才看得到**，用测试客户端永远碰不到 uvicorn 那一层。

### 它慢在哪（**慢是结构性的，优化不掉，请不要因为它慢就删掉它**）

- 要真的把 uvicorn 起起来（每回 4 个工作进程），一共起 5 回，每回一两秒；
- 模拟生图**故意每张 sleep 1.2 秒**（`services/provider.py`，为了让前端进度条
  像真的）。要量「同时在跑几张」，就得真的让它跑一会儿；
- 「领导者被 `kill -9` 之后谁来接手」这一项**必须真的等租约过期**。
  租约 30 秒是产品决定（写在 `services/worker.py` 文件头，也写在
  `docs/deploy-server.md` 里给运营看），测试无权把它调小——
  调小就等于没测那个会上线的数字。光这一项就要等 40 多秒。

对照一下代价：它挡住的是「每张图多花一倍的钱」和「服务器重启后再也不出图」，
**两件事都不会报错，手工点一遍全都正常**。等一分多钟很划算。

### 一条已知问题（这一组**故意**没有断言它）

**全新安装的第一次启动**（数据库文件还不存在）开多进程时，日志里会有几段红字：
...
四个进程同时抢着建表、抢着写初始管理员账号。uvicorn 会把崩掉的子进程重新拉起来，
第二次就都在了，所以**服务照常能用**——线上的表现是「第一次启动有几段红字，
之后一切正常」。这一组现在断言的是**最终状态对不对**（管理员只有一个、
示例模板只写了一份、表都建齐了、四个进程最终都在、healthz 通），
外加**第二次启动零报错**。等哪天有人给这两处补上锁，请回来在
`ColdStartRaceTests` 里加一条「冷启动日志里不许出现 Traceback」，
并把文件开头那段说明删掉。
