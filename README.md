...
> 实测（2026-08-09，自建 Sub2API 网关 + gpt-image-2）：该网关**忽略 size 参数**，
> 按输入图方向吸附到自己支持的比例。同一张 2:1 输入图请求 1:1 方图——
> 关闭预处理输出 1536×1024（3:2，不受控），开启后输出 1254×1254（1:1，符合预期）。
> **所以要控制出图比例，这个开关必须开着。** 单张耗时约 20 秒。
