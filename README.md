...
**商品图会自动预处理**（建议保持开启）：发送前按所选比例补白边；透明底 PNG 自动
合成白底（避免变黑块）；iPhone/Mac 的 HEIC 照片自动转码；手机竖拍照片按方向摆正。
...
**提示词库（含自带模板和灵感库）都只是「风格参考」，不会原样发给生图模型。**
每次生成前，系统会让带视觉的文本模型（默认 `gpt-5.6-sol`）：

1. 看一眼你上传的商品图，认出品类、外形、颜色、材质、品牌
2. 结合你选的模板风格 + 你填的「补充要求」
3. 现场写出一条专为这件商品的提示词，再交给生图模型
...
- 每次多花约 30 秒（文本模型开销远低于生图）
- 在「生成记录」的详情里能看到 AI 实际发出的提示词与你的原始配置
- 文本模型调用失败时自动回退到原提示词，不会挡住出图
- 可在「系统设置 → 生图服务」关闭，或更换文本模型（需支持看图）
